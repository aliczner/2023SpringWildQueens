#bayesmove at 1 day time period for spring 2023 project
...
#one day survey perioud

tracks.day <-round_track_time(dat=tracks.rare, id="id", int=43200, tol=43200, 
...
dist.bin.lims = quantile(tracks.day[tracks.day$dt==43200,]$step,
...
tracks.day.list2 <- map(tracks_disc.list.day,
                         subset,
                         select = c(id, SL, TA))
...
dat.day.res <- segment_behavior(data = tracks.day.list2, 
                                 ngibbs = ngibbs, 
                                 nbins = nbins,
                                 alpha = alpha)
...
tracks.seg.day <- assign_tseg(dat = tracks_disc.list.day, 
                               brkpts = brkpts.day)
head(tracks.seg.day)

# Select only id, tseg, SL, TA,
tracks.seg.day.2 <- tracks.seg.day[,c("id","tseg","SL","TA")]

# Summarize observations by track segment
nbins <- c(7,8)

obs <- summarize_tsegs(dat = tracks.seg.day.2, nbins = nbins)
...
cumsum(theta.means.day) #first 2 states comprise 99.5% of all observations
...
theta.estim.long.day <- expand_behavior(dat = tracks.seg.day, 
                                         theta.estim = theta.estim.day, 
                                         obs = obs,
                                         nbehav = 2, 
...
write.csv(tracks.out.day, "tracksout_rareday.csv")
